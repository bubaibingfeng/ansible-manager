...
def writeini(master_ip='',node_ip=[]):
        ...
        data += '[master:vars]\n'
        if master_ip != '':
            vs = VM.objects.get(vm_ip=master_ip)
            # for v in vs:
            #         if v.varName == 'master:vars':
            data += 'ansible_ssh_pass=%s\n' % vs.vm_ssh_pass
            data += 'ansible_become_pass=%s\n' % vs.vm_ssh_pass
            data += 'ansible_ssh_port=%s\n' % vs.vm_ssh_port
            data += 'ansible_ssh_user=%s\n' % vs.vm_ssh_user
    
    # Handle vars for node group
        data += '[node:vars]\n'
        data += 'ansible_ssh_pass=HTuser123\n' 
        data += 'ansible_become_pass=HTuser123\n' 
        data += 'ansible_ssh_port=22\n' 
        data += 'ansible_ssh_user=userlocal\n' 

        if master_ip != '':
            # Handle vars for master-init group
            data += '[master-init:vars]\n'
            vs = VM.objects.get(vm_ip=master_ip)
            # for v in vs:
            #         if v.varName == 'master:vars':
            data += 'ansible_ssh_pass=%s\n' % vs.vm_ssh_pass
