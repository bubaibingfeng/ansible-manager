...
'''
def update_vm_state(vm_name,request):
    vm = VM.objects.get(vm_name=vm_name)
...
class AnsibleTaskDetail(LoginRequiredMixin,DetailView):
    model = AnsibleTasks
class KVMList(LoginRequiredMixin,ListView):
    model = KVM
    def render_to_response(self, context, **response_kwargs):
...
class KVMDetailView(LoginRequiredMixin,DetailView):
    model = KVM
    slug_field = 'vm_name'
...
class KVMList2(LoginRequiredMixin,ListView):
    model = KVM

class KVMDetail(LoginRequiredMixin,DetailView):
    model = KVM
    # def render_to_response(self, context, **response_kwargs):
    #     kvm = self.get_object()
    #     data = {
...
class VMDetailView(LoginRequiredMixin,DetailView):
    model = VM
    slug_field = 'vm_name'
    slug_url_kwarg = 'vm_name'
    ...
    def render_to_response(self, context, **response_kwargs):
        ...
        simplified_data = {
            ...
            'vm_ssh_pass': data['fields']['vm_ssh_pass'],
            'vm_ssh_port': data['fields']['vm_ssh_port'],
            'vm_cpu': data['fields']['vm_cpu'],
            'vm_memory': data['fields']['vm_memory'],
            'vm_disk': data['fields']['vm_disk'],
            # 添加其他你想要包含的字段
        }
        
        return JsonResponse(simplified_data)
class ClusterList(LoginRequiredMixin,ListView):
    model = Cluster
    def render_to_response(self, context, **response_kwargs):
        update_kvm_ip_reachability()
        tasks = self.get_queryset()  
...
class ClusterDetailView(LoginRequiredMixin,DetailView):
    ...
    def render_to_response(self, context, **response_kwargs):
        ...
        simplified_data = {
            ...
            'master_cpu' : master_vm.vm_cpu,
            'master_memory' : master_vm.vm_memory,
            'master_addtional_disk' : master_vm.vm_disk,
            # 添加其他你需要的字段
        }
        for n in cluster.node_ip:
            k=VM.objects.get(vm_ip=n)
            simplified_data[n+'-node_cpu']=k.vm_cpu
            simplified_data[n+'-node_memory']=k.vm_memory
            simplified_data[n+'-node_addtional_disk']=k.vm_disk
        # 返回简化后的数据为 JSON 响应
        return JsonResponse(simplified_data, **response_kwargs)

class ClusterStateDetailView(LoginRequiredMixin,DetailView):
    model = Cluster
    slug_field = 'cluster_name'
    slug_url_kwarg = 'cluster_name'
    def render_to_response(self, context, **response_kwargs):
        ...
        try:
            ...
            return JsonResponse({'error': 'Master VM not found'}, status=404)
        update_vm_state(master_vm.vm_name, self.request)
        # 将 Cluster 和 VM 信息整合为简化的数据
        simplified_data = {
            'cluster_name': cluster.cluster_name,
            'master_ip': cluster.master_ip,
        ...
        state=State.objects.get(vm_name=master_vm.vm_name)
        simplified_data['master_cpu'] = state.vm_cpu_usage
        simplified_data['master_memory'] = state.vm_memory_usage
        simplified_data['master_disk'] = state.vm_disk_usage
        simplified_data['master_state_updatetime'] = state.update_time
        for n in cluster.node_ip:
            k=VM.objects.get(vm_ip=n)
            update_vm_state(k.vm_name, self.request)
            state=State.objects.get(vm_name=k.vm_name)
            simplified_data[n+':node_disk'] = state.vm_disk_usage
            simplified_data[n+':node_cpu'] = state.vm_cpu_usage
            simplified_data[n+':node_memory'] = state.vm_memory_usage
            simplified_data[n+':node_state_updatetime'] = state.update_time
        # 返回简化后的数据为 JSON 响应
        return JsonResponse(simplified_data, **response_kwargs)